...
const StoryViewer = ({ viewStory, setViewStory }) => {
  ...
  useEffect(() => {
    ...
    if (viewStory && viewStory.media_type !== "video") {
        ...
        const duration = 5000; // 5 seconds for non-video stories
        const setTime = 100;
        let elapsed = 0;
  ...
  const renderContent = () => {
    switch (viewStory.media_type) {
      ...
      case "text":
        return (
          <div className="w-full h-full flex items-center justify-center p-8 text-2xl text-center text-white">
            {viewStory.content}
          </div>
        );
  ...
  return (
    <div
      className="fixed inset-0 h-screen bg-black bg-opcity-90 z-110 flex items-center justify-center "
      style={{
        backgroundColor:
